        ...
        self.addEventListener(
            ...
            (event) => {
                if (event.origin !== origin) {
                    reject(
                        new Error(`Unexpected message from ${event.origin}`)
                    );
                    return;
                }
                ...
                if (event.data.action === request.action) {
                    ...
                    reject(
                        new Error(
                            `Unexpected message ${JSON.stringify(event.data)}`
                        )
                    );
                }
            },
...
for (const databaseKind of ["open", "closed"]) {
    for (const mode of ["iframe", "window"]) {
        promise_test(async (testCase) => {
            ...
            assert_true(
                await crossOriginHelper(testCase, mode, sameOrigin, {
            ...
            assert_true(
                await crossOriginHelper(testCase, mode, otherOrigin, {
                    action: "delete-database",
                    name: dbName,
                }),
            ...
            if (databaseKind === "closed") await db.close();

            const sameOriginDbNames = await crossOriginHelper(
                testCase,
                mode,
                sameOrigin,
                { action: "get-database-names" }
            );
            assert_in_array(
                sameOriginDbNames,
                dbName,
