...
indexeddb_test(
    ...
    function (t, db) {
        var store = db
            .transaction("Store1", "readwrite", { durability: "relaxed" })
            .objectStore("Store1");
...
indexeddb_test(
    ...
    function (t, db) {
        var store = db
            .transaction("Store2", "readwrite", { durability: "relaxed" })
            .objectStore("Store2");
        store.openCursor().onsuccess = t.step_func(function (e) {
            var item = e.target.result.value;
            store.index("CompoundKey").get([item.num, item.id]).onsuccess =
...
indexeddb_test(
    function (t, db, txn) {
        // Auto-increment
        var store = db.createObjectStore("Store3", {
            keyPath: "id",
            autoIncrement: true,
        });
    ...
    function (t, db) {
        var store = db
            .transaction("Store3", "readwrite", { durability: "relaxed" })
            .objectStore("Store3");
        const keys = [];
        let count;
